...
use \Closure;
use Dasigr\Commerce\Money;
use Dasigr\Commerce\Exception\InvalidProductException;

class Product extends Helpper {
    ...
    {
        if ($this->taxable) {
            $total = $this->value->substract($this->discount);
    ...
    {
        if (is_int($value)) {
            $this->discount = Money::init($value, $this->currencty);
            return $this->tax = $this->calculateTax();
        }
